...
//! Drop‑in replacement: adds a `:shell` meta command that calls the interpreter's
//! `Executor::enter_shell(&mut env)` adapter and keeps all original behavior.
...
fn handle_meta(cmd: &str, executor: &mut Executor) -> Result<()> {
    match cmd {
        ...
        ":shell" => {
            // Executor::enter_shell should call the shell adapter and return Result<()>
            match executor.enter_shell(&mut executor.env) {
