...
pub fn eval_stmt(exec: &mut Executor, stmt: &Statement) -> Result<Option<Value>> {
    ...
    match stmt {
        ...
        // ── FOR x IN iterable ─────────────────────────────────────────────────
        Statement::ForIn { var, iterable, body, span } => {
        ...
        Statement::RetLate { value, condition, span: _ } => {
            ...
            let deliver_at_ms = match condition {
                ...
                RetLateCondition::WhenTrue(_trigger_expr) => 0u64,
            };

            let pending = Value::Pending(Box::new(snapshot), deliver_at_ms);
            exec.env.set_local("__ret_late__".to_string(), pending);
            ex_frame::pop_frame(&mut exec.traceback);
            Ok(None)
        }
        ...
        Statement::AttemptBlock { err_var, try_body, else_body, span: _ } => {
            let try_result: Result<(), anyhow::Error> = (|| {
...
pub fn eval_expr(exec: &mut Executor, expr: &Expr) -> Result<Value> {
    match expr {
        ...
        Expr::Call { callee, args, .. } => {
                ...
                if let Expr::Identifier(id) = &**callee {
                    ...
                    if name_lc.starts_with("ret.now") || name_lc == "ret.now" {
                        ...
                        let ret_concrete = exec.deref(ret_val);
            ...
            if let Expr::Identifier(id) = &**callee {
                ...
                if let Some((func_params, func_body)) = fn_lkp {
                    ...
                    let pending = exec.env.get("__ret_late__");
                    if let Some(p) = pending { last = exec.deref(p); }
                    ex_frame::pop_scope(&mut exec.env, "fn call", &mut exec.diagnostics);
                    return Ok(last);
                }

                // 2. Lambda stored in environment
                let maybe_lambda = exec.env.get(&id.name)
                    .or_else(|| exec.env.get(&name))
                    .map(|v| exec.deref(v));

                if let Some(Value::Lambda(stmts)) = maybe_lambda {
                    ex_frame::push_scope(&mut exec.env);
                    for (i, val) in argvals.iter().enumerate() {
                        ex_frame::set_local(&mut exec.env, format!("__arg_{}__", i), val.clone());
                    }
                    let mut last = Value::None;
                    for s in stmts.iter() {
                        if let Some(v) = exec.execute_statement(s)? { last = v; }
        ...
        Expr::Lambda(stmts, _) => {
            let id = exec.gc.allocate(Value::Lambda(stmts.clone()));
            Ok(Value::Heap(id))
        }

        Expr::Raw(_, _) => Ok(Value::None),

        Expr::TensorBuilder { expr, .. } => exec.build_tensor(expr),
        ...
        Expr::Index { base, indices, .. } => {
            ...
            let idx = match idx_val {
                ...
                other => return Err(anyhow!("Index must be a number, got {:?}", other)),
            };
            match base_val {
                Value::List(items) => {
