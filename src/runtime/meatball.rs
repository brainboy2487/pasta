...
#[derive(Debug, Clone)]
pub enum Message {
    /// A Pasta runtime `Value` passed between meatballs.
    Value(Value),
}
...
pub struct MeatballHandle {
    /// Unique identifier for this meatball instance.
    pub id: MeatballID,
...
pub struct Meatball {
    /// Unique identifier for this meatball instance.
    pub id: MeatballID,
    /// The interpreter instance owned by this meatball.
...
impl Saucepan {
        ...
        F: FnOnce(&mut Executor, Receiver<Message>, Sender<Message>) -> Result<()> + Send + 'static,
    {
        ...
        let handle_thread = thread::spawn(move || {
            ...
            f(&mut meatball.executor, meatball.inbox, outbox)
        });
